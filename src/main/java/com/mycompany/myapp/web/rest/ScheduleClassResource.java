...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.ScheduleClass}.
 */
@RestController
@RequestMapping("/api")
public class ScheduleClassResource {
    ...
    public ScheduleClassResource(ScheduleClassService scheduleClassService) {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new scheduleClass, or with status {@code 400 (Bad Request)} if the scheduleClass has already an ID.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PostMapping("/schedule-classes")
    public ResponseEntity<ScheduleClass> createScheduleClass(@Valid @RequestBody ScheduleClass scheduleClass) throws URISyntaxException {
        log.debug("REST request to save ScheduleClass : {}", scheduleClass);
        if (scheduleClass.getId() != null) {
            throw new BadRequestAlertException("A new scheduleClass cannot already have an ID", ENTITY_NAME, "idexists");
        }
        ScheduleClass result = scheduleClassService.save(scheduleClass);
        return ResponseEntity.created(new URI("/api/schedule-classes/" + result.getId()))
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated scheduleClass,
     * or with status {@code 400 (Bad Request)} if the scheduleClass is not valid,
     * or with status {@code 500 (Internal Server Error)} if the scheduleClass couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/schedule-classes")
    public ResponseEntity<ScheduleClass> updateScheduleClass(@Valid @RequestBody ScheduleClass scheduleClass) throws URISyntaxException {
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, scheduleClass.getId().toString()))
            .body(result);
    }
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of scheduleClasses in body.
     */
    @GetMapping("/schedule-classes")
    public List<ScheduleClass> getAllScheduleClasses(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the scheduleClass, or with status {@code 404 (Not Found)}.
     */
